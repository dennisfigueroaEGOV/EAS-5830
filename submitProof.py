...
def convert_leaves(primes_list):
    ...
    for prime_number in primes_list:
        prime_bytes32 = prime_number.to_bytes(32, 'big')
        primes_list_bytes32.append(prime_bytes32)

    return primes_list_bytes32
...
def prove_merkle(merkle_tree, random_indx):
    ...
    for level in merkle_tree[:-1]:
        ...
        partner_index = current_index + 1 if current_index % 2 == 0 else current_index - 1

        if partner_index < level_size:
            proof_element = level[partner_index]
            merkle_proof.append(proof_element)
...
def send_signed_msg(proof, random_leaf):
    ...
    # proof_hex = [element.hex() if isinstance(element, bytes) else element for element in proof]

    proof_bytes32 = [Web3.to_bytes(hexstr=element.hex().zfill(64)) if isinstance(element, bytes) else Web3.to_bytes(hexstr=element.zfill(64)) for element in proof]
    random_leaf_bytes32 = Web3.to_bytes(hexstr=random_leaf.hex().zfill(64)) if isinstance(random_leaf, bytes) else Web3.to_bytes(hexstr=random_leaf.zfill(64))

    transaction = contract.functions.submit(proof_bytes32, random_leaf_bytes32).build_transaction({
        'from': acct.address,
